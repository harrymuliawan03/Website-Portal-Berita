  ...
  <div class="card mb-3">
    @if ($posts[0]->image)
      <div style="max-height: 500px; overflow:hidden;">
        <img src="{{ asset('storage/' . $posts[0]->image) }}" alt="{{ $posts[0]->category->category_name }}" class="img-fluid w-100">
      </div>
    @else
    ...
    <div class="card-body text-center">
     ...
     <p>
            ...
            By. <a href="/posts?author={{ $posts[0]->author->username }}" class="text-decoration-none">{{ $posts[0]->author->name }}</a> in <a href="/posts?category={{ $posts[0]->category->category_name }}" class="text-decoration-none">{{ $posts[0]->category->category_name }}</a> {{ $posts[0]->created_at->diffForHumans() }}
            </small>
    </p>
      ...
      <a href="/posts/{{ $posts[0]->slug }}@if(request('author'))?author={{ request('author') }} @elseif(request('category'))?category={{ request('category') }} @endif" class="text-decoration-none">Read more</a>
    </div>
  </div>
